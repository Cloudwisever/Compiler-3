...
{
	SymbolItem read, write, writing_;
	read = (SymbolItem)malloc(sizeof(struct SymbolItem_));
	memset(read, 0, sizeof(struct SymbolItem_));
	
	strcpy(read->name, "read");
	read->SymbolType = stdINT;
	read->initialized = 1;
	read->kind = FUNC;
	
	SymbolTable_Add(sym_table, read);
	
	write = (SymbolItem)malloc(sizeof(struct SymbolItem_));
	memset(write, 0, sizeof(struct SymbolItem_));
	writing_ = (SymbolItem)malloc(sizeof(struct SymbolItem_));
	memset(writing_, 0, sizeof(struct SymbolItem_));
	strcpy(writing_->name, "writing_content_");
	writing_->SymbolType = stdINT;
	writing_->kind = FUNCDEFPARAM;
	
	strcpy(write->name, "write");
	write->SymbolType = stdINT;
	write->initialized = 1;
	write->next = writing_;
	write->kind = FUNC;
	
	SymbolTable_Add(sym_table, write);
}
...
{	
	...
	else if ( strcmp(spe_root->children[1]->name, "OptTag") == 0)//StructSpecifier, declare a structure
	{
		...
		else if(spe_root->children[1]->children[0] != NULL)//Opttag is not empty
		{
			struct_name = spe_root->children[1]->children[0]->content;
	
			newone->size = FieldListFillMaster(tmp, struct_name);
...
int FieldListFillMaster(FieldList field, char* master)
{
	...
	while(field != NULL)
	{
		...
		tmp->symbol->offset = field_off;
		field_off += tmp->symbol->size;
...
SymbolItem HandleDec(Syntax_Leaf* root, Type decType, int deflist_type)
{
	...
	else
	{
		...
		else if(deflist_type == ARRAY)
		{
			printf("Error type 15 at Line %i: trying to initialize an array.\n", var->lineno);
		}
	}
	return var;
}
...
SymbolItem HandleVarDec(Syntax_Leaf* var_root, Type decType, int deflist_type)
{
	...
	while(root->children[0]->type == TOKEN_)
	{
		...
		array_sum_size = array_size*array_sum_size;
	...
	if(root->children[0]->type == ID_)
	{
		...
		newid->size = array_sum_size*decType->size;
