...
InterCodes translate_Dec(Syntax_Leaf* root, HashItem* sym_table)
{
	...
	else if((strcmp(vardec->children[0]->name, "Exp") == 0)&&(strcmp(vardec->children[1]->name, "LB") == 0)&&(strcmp(vardec->children[2]->name, "Exp") == 0)&&(strcmp(vardec->children[3]->name, "RB") == 0))//exp[exp]
	{
		good = 2;
	}
	printf("good = %i\n", good);
	...
	else
	{
		if(good == 2)
		{
			...
			else
			{
				...
				ir.kind = DEC;
				ir.u.assign.left = opID;
				ir.u.assign.right = getConstant(idhash->symbol->size);
				code1 = addIRhead(ir, code1);
...
InterCodes translate_Exp(Syntax_Leaf* root, HashItem* sym_table, Operand place)
{
	...
	else if(root->children[0]->type == TOKEN_)//Exp : Exp ...
	{
		...
		{
			if(exp1_root->children[0]->type != ID_ || exp2_root->children[0]->type != INT_)
			{
			...
			ir.u.binop.op2 = getConstant(idhash->symbol->SymbolType->size*num);
